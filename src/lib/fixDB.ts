...
export default async (knex: Knex): Promise<void> => {
  ...
  const hasTextModelTable = await knex.schema.hasTable("t_textModel");
  if (hasTextModelTable) {
    const oauthModel = await knex("t_textModel").where({ manufacturer: "chatgptOauth", model: "gpt-5.4" }).first();
    if (!oauthModel) {
